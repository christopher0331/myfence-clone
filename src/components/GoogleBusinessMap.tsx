...
// Must match AddressAutocomplete (and any other caller) so @react-google-maps/api
// de-dupes to a single script load. Changing this tuple will re-trigger loader
// collisions on page refresh.
...
const GoogleBusinessMap = (props: GoogleBusinessMapProps) => {
  const { className = "", city, state, radiusMiles } = props;
  ...
  return <GoogleBusinessMapInner {...props} apiKey={apiKey} />;
};

const GoogleBusinessMapInner = ({
  placeId,
  city,
  state,
  radiusMiles,
  ...
  zoom: customZoom,
  apiKey,
}: GoogleBusinessMapProps & { apiKey: string }) => {
